...
class DAT_RUL(nn.Module):
    def __init__(self, cell_type, sequence_len, feature_num, hidden_dim, lstm_dim,
                 ...
                 fc_dropout=0.1, rnn_dropout=0.1, bidirectional=False, return_attention_weights=False, alpha=0.5, beta=0.5):
        super(DAT_RUL, self).__init__()
        assert cell_type in ['rnn', 'lstm', 'gru', 'grucell', 'slstm', 'mlstm', 'xlstm']
        assert fc_activation in ['tanh', 'gelu', 'relu', 'silu', 'leakyrelu', 'elu']
        ...
        if attention_type == 'deg_attention':
            ...
            self.attention_enc = DAT(sequence_len, feature_num, hidden_dim,
        ...
        #     nn.Linear(hidden_dim * 2 if bidirectional else lstm_dim, fc_layer_dim),
        ...
        #                num_experts=4,
        #                experts=experts)
        self.decoder = nn.Sequential( nn.Linear( hidden_dim, hidden_dim*4 ),
                                      ACTIVATION_MAP[fc_activation](),
                                      # nn.Linear( args.fc_layer_dim, args.fc_layer_dim ), ACTIVATION_MAP[self.fc_activation]()( inplace = True ),
                                      nn.Linear( hidden_dim*4, 1 ) )

        self.channel_decoder = nn.Sequential( nn.Linear( lstm_dim, lstm_dim*4 ),
                                              ACTIVATION_MAP[fc_activation](),
                                              nn.ReLU(), nn.Linear( lstm_dim*4, 1 ) )
    ...
    def forward(self, x, idx=None):
        ...
        # fc_output = self.channel_decoder(dropout_output)

        # moe_out,_ = self.moe(dropout_fused_attention.permute(0, 2, 1))
        # dropout_output = self.dropout(rnn_output)  # output_mean = dropout_output.mean(dim=1)
        # decoder_output = self.decoder( dropout_output)
        # fc_output = self.channel_decoder( decoder_output.permute( 0, 2, 1 ) )

        decoder_output = self.decoder( dropout_output.permute( 0, 2, 1 ) )
        ...
        if self.return_attention_weights:
            return fused_attention, rul_prediction,
        else:
