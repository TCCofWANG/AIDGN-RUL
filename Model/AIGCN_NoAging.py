...
class AIGCN_NoAging( nn.Module ) :
    def __init__(self, args, supports=None) :
        super(AIGCN_NoAging, self).__init__()
        ...
        for _ in range( self.n_layer ) :
            ...
            self.m_gate1.append( nn.Sequential( nn.Linear( args.sequence_len, 1 ),nn.ReLU()) )
            self.m_gate2.append( nn.Sequential( nn.Linear( args.sequence_len, 1 ), nn.ReLU() ))
    ...
    def forward(self, x, OCC=None) :
        ...
        for layer in range( self.n_layer ) :
            ...
            m1 = self.m_gate1[layer]( torch.cat( [x], dim = -1 ) ) # (B, C, M, L_bt)
            m2 = self.m_gate2[layer]( torch.cat( [x], dim = -1 ) ) # (B, C, M, L_bt)
            ...
            e1 = e1.permute( 0, 2, 1, 3 )  # (B, M, C, L_B)
            e2 = e2.permute( 0, 2, 3, 1 )  # (B, M, L_B, C)

            adp = F.softmax( nn.ReLU()( torch.matmul( e1, e2 ) ),  dim = -1 )  # (B, M, C, C) used 32 1 16 16
