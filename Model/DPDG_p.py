...
class ACE ( nn.Module ) :
    def __init__(self , layers , sequence_len , AATE_dim ,dropout, single_path=False) :
        ...
        for _ in range ( self.n_layer ) :
            ...
            self.m_gate1.append ( nn.Sequential ( nn.Linear ( sequence_len + self.AATE_dim , 1 ) , nn.ELU ( ) ) )
            self.m_gate2.append ( nn.Sequential ( nn.Linear ( sequence_len + self.AATE_dim , 1 ) , nn.ELU ( ) ) )
    ...
    def forward(self , layer , x , AATE , AATE_T) :
        ...
        if not self.single_path :
            ...
            eT = self.dropout ( F.softmax ( F.elu_ ( mT * self.ll2[layer] ( x ) ) , dim = -1 ) )  # (B, C, M, AATE_dim)
            eT = AATE_T + eT.permute ( 0 , 2 , 3 , 1 )  # (B, M, AATE_dim, C)
        else:
            eT = e.permute ( 0 , 1 , 2 , 3 )
        return F.softmax ( F.elu_ ( torch.matmul ( e , eT ) ) , dim = -1 )  # (B, M, C, C)
...
class gcn ( nn.Module ) :
    ...
    def forward(self , x , supports , dyn_corr=None , kappa=None , alpha=None) :
        ...
        diffusion_corr, s_t, rhs = self.diffusion ( x , A_for_diffusion , dyn_corr )

        return s_t, rhs, h + diffusion_corr


class DPDG_p ( nn.Module ) :
    def __init__(self , args , supports=None) :
        ...
        # Aging-context
        self.ACE =ACE(self.n_layer, self.sequence_len, self.AATE_dim, self.dropout)
        # Graph + output layers
        self.gconv = nn.ModuleList ( [gcn ( args.sequence_len , self.hidden_dim , self.dropout , self.feature_num ,
    ...
    def forward(self , x_in , OCC=None) :
        physics_state=diffusion_rhs=None
        ...
        for layer in range ( self.n_layer ) :
            ...
            physics_state, diffusion_rhs, x = self.gconv[layer] ( x.permute ( 0 , 3 , 1 , 2 ) , new_supports , dyn_corr , self.kappa ,  self.alpha )  # (B, H, C, M)
            x = F.elu_ ( self.dropout ( x ) ).permute ( 0 , 2 , 3 , 1 )  # (B, C, M, H)
            if layer > 0 : x = x + x_last

        x = x.mean ( dim = 2 ) if self.outlayer == "Linear" else x.view ( B * C , self.M , -1 ).permute ( 0 , 2 , 1 )  # (B, C, H)
        ...
        return physics_state, diffusion_rhs, rul_prediction
