...
class DiffusionLayer(nn.Module):
    def __init__(self, node_count, out_channels, dt=1.0, use_dynamic_encode=True):
    ...
    def forward(self, x, A, dyn_porior=None):
        ...
        if self.use_dynamic_encode and dyn_porior is not None:
            dyn_corr = self._alpha * dyn_porior  # scaled contribution
        else:
            dyn_corr = 0.0

        # Euler update
        s_new = s - self.dt * kappa * Ls + self.dt * (r_out + dyn_corr)
...
class gcn(nn.Module):
    ...
    def forward(self, x, supports, dyn_corr=None):
        B, F, C, M = x.shape
        out = [x]
        for b in supports:
            x1 = self.nconv(x, b)
...
class PIGDN(nn.Module):
    def __init__(self, args, supports=None):
        ...
        if self.use_dynamic_encoder:
            self.dynamic_encoder = nn.Sequential(
                ...
                nn.ELU(),
                nn.Conv1d(self.hidden_dim, self.feature_num, kernel_size=1),
                nn.AdaptiveAvgPool1d(self.M)
            )
            # self.dynamic_encoder = nn.Sequential(
            #     nn.Conv1d(self.feature_num, self.hidden_dim, kernel_size=3, padding=1),
            #     nn.ELU(),
    ...
    def forward(self, x, OCC=None):
        ...
        dyn_corr = None
        if self.use_dynamic_encoder:
            dyn_corr = self.dynamic_encoder(x.squeeze(2))  # (B, C)
            # dyn_corr = dyn_corr.unsqueeze(-1).expand(-1, -1, self.M)  # (B, C, M)
        ...
        for layer in range(self.n_layer):
            ...
            x = self.gconv[layer](x.permute(0, 3, 1, 2), new_supports, dyn_corr)
