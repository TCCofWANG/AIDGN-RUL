...
class DVGTformer(nn.Module):
    def __init__(self, args, input_feature):
        super(DVGTformer, self).__init__()
        #num_nodes, time_length, d_model, num_heads, lambda_param, d_ff, dropout, num_blocks):
        ...
        self.linear_t = nn.Linear(args.input_length, args.input_length)
        self.linear_x = nn.Linear(input_feature, input_feature)

        self.t_v = nn.Parameter(torch.randn(1, 1, input_feature))
        self.x_v = nn.Parameter(torch.randn(1, args.input_length + 1, 1))

        self.positional_encoding = self.create_positional_encoding(args.input_length + 1, input_feature + 1)

        self.tvgtformer_blocks = nn.ModuleList(
            [TVGTformer(input_feature, args.input_length, self.d_model[0], self.num_heads, self.lambda_param, args.d_ff[0], args.dropout) for _ in
             range(self.num_blocks)])
        self.svgtformer_blocks = nn.ModuleList(
            [SVGTformer(input_feature, args.input_length, self.d_model[1], self.num_heads, self.lambda_param, args.d_ff[1], args.dropout) for _ in
             range(self.num_blocks)])


        self.output_layer = nn.Sequential(
            nn.Linear(((args.input_length + 1) * (input_feature + 1)), 100),
            nn.GELU(),
            nn.Linear(100, 1)
        )
