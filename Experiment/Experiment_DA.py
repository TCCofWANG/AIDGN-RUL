...
class Exp_DA(object):
    ...
    def _get_model(self):
        ...
        elif 'AIGCN' == self.args.model_name :
    ...
    def _get_data(self, flag="train"):
        ...
        if self.args.dataset_name == 'CMAPSS':
            s_x_train, t_x_train,s_i_train, t_i_train, s_x_vali, t_x_vali, s_i_vali, t_i_vali, s_y_vali, t_y_train, t_y_vali, X_test, y_test, index_test = da_get_cmapss_data_(data_path=args.data_path_CMAPSS,
        ...
        s_train_data_set = eval(self.args.dataset_name+'Data')(s_x_train, s_i_train, t_y_train)
        t_train_data_set = eval(self.args.dataset_name+'Data')(t_x_train,t_i_train, t_y_train)

        s_vali_data_set = eval(self.args.dataset_name+'Data')(s_x_vali, s_i_vali, t_y_vali)
        t_vali_data_set = eval(self.args.dataset_name+'Data')(t_x_vali, s_y_vali, t_y_vali)
        ...
        if self.args.train:
            ...
            for epoch in range(self.args.train_epochs):
                ...
                for _, (unpacked1, unpacked2) in enumerate(tqdm(zip(self.s_train_loader, self.t_train_loader), total=min(len(iter(self.s_train_loader)), len(iter(self.t_train_loader))))):
                    ...
                    s_features, s_out = self.model(s_d, s_i)
                    t_features, t_out = self.model(t_d, t_i)
    ...
    def validation(self, s_vali_loader, t_vali_loader, criterion):
        ...
        for _, (unpacked1, unpacked2) in enumerate(tqdm(zip(self.s_vali_loader, self.t_vali_loader), total=min(len(iter(self.s_vali_loader)), len(iter(self.t_vali_loader))))):
            ...
            s_d, s_idx, batch_y = unpacked1
            t_d, t_idx, _ = unpacked2

            s_d = s_d.double().to(self.device)  # [B,window_size,D]
            t_d = t_d.double().to(self.device)  # [B,window_size]
            batch_y = batch_y.double().to(self.device)

            s_features, s_out = self.model(s_d, s_idx)
            t_features, t_out = self.model(t_d, t_idx)
    ...
    def test(self, test_loader):
        ...
        enc_pred = []
        gt = []
        for i, (batch_x,s_idx,  batch_y) in enumerate(tqdm(test_loader)):
            batch_x = batch_x.double().to(self.device)
            batch_y = batch_y.double().to(self.device)
            s_idx = s_idx.double().to(self.device)

            if self.args.is_minmax:
                t_features, t_out = self.model(batch_x, s_idx)
                outputs = t_out * self.max_life

            else:
                t_features, t_out = self.model(batch_x, s_idx)  # outputs[B,window_size]
                outputs = t_out
